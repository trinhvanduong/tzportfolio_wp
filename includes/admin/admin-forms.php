...
if ( ! class_exists( 'tp\admin\Admin_Forms' ) ) {
	...
	class Admin_Forms {
		...
		function render_form( $echo = true ) {
			...
			$class = 'form-table tp-form-table ' . ( ! empty( $this->form_data['class'] ) ? $this->form_data['class'] : '' );
		...
		function render_form_row( $data ) {
			...
			if ( $data['type'] != 'hidden' ) {
				...
				if ( ! empty( $this->form_data['div_line'] ) ) {
					...
					if ( strpos( $this->form_data['class'], 'tp-top-label' ) !== false ) {

						$html .= '<div class="form-field tp-forms-line" ' . $conditional . $prefix_attr . $type_attr . '>' . $this->render_field_label( $data );
						...
						if ( ! empty( $data['without_label'] ) ) {
							...
							$html .= '<div class="form-field tp-forms-line" ' . $conditional . $prefix_attr . $type_attr . '>';
					...
					if ( strpos( $this->form_data['class'], 'tp-top-label' ) !== false ) {
						...
						$html .= '<tr class="tp-forms-line" ' . $conditional . $prefix_attr . $type_attr . '>
						...
						if ( ! empty( $data['description'] ) )
							$html .= '<div class="tp-admin-clear"></div><p class="description">' . $data['description'] . '</p>';
		...
		function render_field_by_hook( $data ) {
			 ...
			 *
			 * @type filter
			 * @title tp_render_field_type_{$type}
			 ...
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_filter( 'tp_render_field_type_{$type}', 'function_name', 10, 4 );
			 * @example
			 * <?php
			 * add_filter( 'tp_render_field_type_{$type}', 'my_render_field_type', 10, 4 );
			 ...
			 * ?>
			 */
			return apply_filters( 'tp_render_field_type_' . $data['type'], '', $data, $this->form_data, $this );
		}
		...
		function tooltip( $tip, $allow_html = false, $echo = true ) {
            ...
            <span class="tp_tooltip dashicons dashicons-editor-help" title="<?php echo $tip ?>"><?php echo $tip ?></span>
		...
		function render_hidden( $field_data ) {
			...
			$class_attr = ' class="tp-forms-field ' . $class . '" ';
		...
		function render_text( $field_data ) {
			...
			$class = ! empty( $field_data['class'] ) ? $field_data['class'] : '';
			$class .= ! empty( $field_data['size'] ) ? 'tp-' . $field_data['size'] . '-field' : 'tp-long-field';
			$class_attr = ' class="tp-forms-field ' . $class . '" ';
		...
		function render_icon( $field_data ) {
			...
			$html = '<a href="#" class="button" data-modal="TP_fonticons" data-modal-size="normal" data-dynamic-content="tp_admin_fonticon_selector" data-arg1="" data-arg2="" data-back="">' . __( 'Choose Icon', 'ultimate-member' ) . '</a>
                <span class="tp-admin-icon-value">';
			...
			if ( get_post_meta( get_the_ID(), '_tp_icon', true ) ) {
				$html .= '<span class="tp-admin-icon-clear show"><i class="tp-icon-android-cancel"></i></span>';
			} else {
				$html .= '<span class="tp-admin-icon-clear"><i class="tp-icon-android-cancel"></i></span>';
			}
		...
		function render_textarea( $field_data ) {
			...
			$class = ! empty( $field_data['class'] ) ? $field_data['class'] : '';
			$class .= ! empty( $field_data['size'] ) ? $field_data['size'] : 'tp-long-field';
			$class_attr = ' class="tp-forms-field ' . $class . '" ';
		...
		function render_multi_selects( $field_data ) {
			...
			$html = "<select class=\"tp-hidden-multi-selects\" $data_attr>$options</select>";
			$html .= "<ul class=\"tp-multi-selects-list\" $data_attr>";
			...
			if ( ! empty( $values ) ) {
				foreach ( $values as $k=>$value ) {
					...
					$html .= "<li class=\"tp-multi-selects-option-line\"><span class=\"tp-field-wrapper\">
                        <select $id_attr $name_attr $class_attr $data_attr>$options</select></span>
                        <span class=\"tp-field-control\"><a href=\"javascript:void(0);\" class=\"tp-select-delete\">" . __( 'Remove', 'ultimate-member' ) . "</a></span></li>";
				}
			...
			$html .= "</ul><a href=\"javascript:void(0);\" class=\"button button-primary tp-multi-selects-add-option\" data-name=\"$name\">{$field_data['add_text']}</a>";
		...
		function render_multi_checkbox( $field_data ) {
			...
			while ( $i < $columns ) {
				...
				$html .= '<span class="tp-form-fields-section" style="width:' . floor( 100 / $columns ) . '% !important;">';
		...
		function render_multi_text( $field_data ) {
			...
			$size = ! empty( $field_data['size'] ) ? 'tp-' . $field_data['size'] . '-field' : 'tp-long-field';
			...
			$html = "<input type=\"text\" class=\"tp-hidden-multi-text\" $data_attr />";
			$html .= "<ul class=\"tp-multi-text-list\" $data_attr>";
			...
			if ( ! empty( $values ) ) {
				foreach ( $values as $k=>$value ) {
					...
					$html .= "<li class=\"tp-multi-text-option-line {$size}\"><span class=\"tp-field-wrapper\">
                        <input type=\"text\" $id_attr $name_attr $class_attr $data_attr value=\"$value\" /></span>
                        <span class=\"tp-field-control\"><a href=\"javascript:void(0);\" class=\"tp-text-delete\">" . __( 'Remove', 'ultimate-member' ) . "</a></span></li>";
				}
			} elseif ( ! empty( $field_data['show_default_number'] ) && is_numeric( $field_data['show_default_number'] ) && $field_data['show_default_number'] > 0 ) {
				$i = 0;
				while( $i < $field_data['show_default_number'] ) {
			...
			$html .= "</ul><a href=\"javascript:void(0);\" class=\"button button-primary tp-multi-text-add-option\" data-name=\"$name\">{$field_data['add_text']}</a>";
		...
		function render_media( $field_data ) {
			...
			$class = ! empty( $field_data['class'] ) ? $field_data['class'] : '';
			$class .= ! empty( $field_data['size'] ) ? $field_data['size'] : 'tp-long-field';
			$class_attr = ' class="tp-forms-field tp-media-upload-data-url ' . $class . '"';
			...
			$html = "<div class=\"tp-media-upload\">" .
			        "<input type=\"hidden\" class=\"tp-media-upload-data-id\" name=\"{$name}[id]\" id=\"{$id}_id\" value=\"$image_id\">" .
			        "<input type=\"hidden\" class=\"tp-media-upload-data-width\" name=\"{$name}[width]\" id=\"{$id}_width\" value=\"$image_width\">" .
			        "<input type=\"hidden\" class=\"tp-media-upload-data-height\" name=\"{$name}[height]\" id=\"{$id}_height\" value=\"$image_height\">" .
			        "<input type=\"hidden\" class=\"tp-media-upload-data-thumbnail\" name=\"{$name}[thumbnail]\" id=\"{$id}_thumbnail\" value=\"$image_thumbnail\">" .
			...
			if ( ! empty( $field_data['url'] ) ) {
				$html .= '<input type="text" class="tp-media-upload-url" readonly value="' . $image_url . '" /><div style="clear:both;"></div>';
			}

			$html .= '<input type="button" class="tp-set-image button button-primary" value="' . __( 'Select', 'ultimate-member' ) . '" data-upload_frame="' . $upload_frame_title . '" />
                    <input type="button" class="tp-clear-image button" value="' . __( 'Clear', 'ultimate-member' ) . '" /></div>';
		...
		function render_ajax_button( $field_data ) {
			...
			$class = ! empty( $field_data['class'] ) ? $field_data['class'] : '';
			$class_attr = ' class="tp-forms-field button ' . $class . '" ';
			...
			$html = "<input type=\"button\" $id_attr $class_attr $name_attr $data_attr $value_attr /><div class='clear'></div><div class='tp_setting_ajax_button_response'></div>";
